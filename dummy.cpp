...
// We have a linked list for routes
class RouteNode
{
public:
    string startingPort;
...
public:
    ...
    // functon for adding the port in the vector
    ...
    // Function for checking direct routes
    ...
    // Function For checking connecting routes
    bool checkConnectingRoutes(string origin, string dest, string date, Vector<int>& path) {
        ...
        Vector<int> parent(ports.getSize());
        ...
        Queue q;
        q.enqueue(org_index);
        visited[org_index] = true;

        bool found = false;

        while (!q.isEmpty() && !found) {
            int current = q.frontfind(); 
            q.dequeue();

            RouteNode* route = ports[current].routeHead;
            while (route != nullptr) {
                ...
                if (!visited[nextindex] && route->departureDate == date) {
    ...
    // Add this function to the Maps class (before the showMap function)
bool findAndDisplayRoute(string origin, string destination, string date, Vector<int>& currentPath, string& resultText, Color& resultColor) {
    currentPath.clear();
    
    // First check for direct route
    bool directFound = checkDirectRoutes(origin, destination, date, currentPath);
    if (directFound) {
    ...
    // If no direct route, check for connecting routes
    bool connectingFound = checkConnectingRoutes(origin, destination, date, currentPath);
    if (connectingFound) {
    ...
    // No route found
    resultText = "✗ No route found for given criteria!";
    resultColor = Color::Red;
    return false;
}

// Then update the showMap function to use this new function
void showMap(Maps &graph) {
    ...
    // Create beautiful search route button at bottom left
    ...
    inputWindow.setPosition(710, 340); // Center of screen
    ...
    // Enhanced input boxes with better spacing
        ...
        void handleEvent(Event& event, RenderWindow& window) {
            ...
            if (event.type == Event::TextEntered && isActive) {
                if (event.text.unicode == '\b') { // Backspace
                    ...
                    // Do nothing or handle enter if needed
                }
                else if (event.text.unicode < 128 && event.text.unicode != '\r' && event.text.unicode != '\t') {
                    // Only allow reasonable input length
    ...
    // Create enhanced input boxes with proper spacing
    ...
    float time = 0.0f;
    vector<float> portPulse(ports.getSize(), 0.0f);
    
    // For route search results
    ...
    while (window.isOpen()) {
        ...
        while (window.pollEvent(event)) {
            ...
            if (event.type == Event::MouseButtonPressed && event.mouseButton.button == Mouse::Left) {
                ...
                // Main search button with beautiful animation
                if (searchButton.getGlobalBounds().contains(mousePos) && !showInputWindow) {
                    showInputWindow = true;
                    // Reset input fields when opening window
                    originInput.setText("");
                ...
                if (showInputWindow) {
                    ...
                    if (popupSearchButton.getGlobalBounds().contains(mousePos)) {
                        ...
                        if (origin.empty() || destination.empty() || date.empty()) {
                            ...
                            // Use the new function to find and display route
                            bool routeFound = findAndDisplayRoute(origin, destination, date, currentPath, searchResultText, searchResultColor);
                            showSearchResult = true;
                            showRoutes = true;
                            
                            // Close window after successful search
                            if (routeFound) {
                ...
                // Check port clicks (only if input window is not open)
                if (!showInputWindow) {
                    for (int i = 0; i < numLocations; i++) {
                        float px = portLocations[i].x;
                        float py = portLocations[i].y;
                        float dist = sqrt(pow(clickPos.x - px, 2) + pow(clickPos.y - py, 2));
                        if (dist < 12) {
                            selectedPortIndex = i;
                            showRoutes = true;
                            portPulse[i] = 1.0f;
            ...
            // Handle text input for input boxes when window is open
            if (showInputWindow) {
                originInput.handleEvent(event, window);
                destinationInput.handleEvent(event, window);
                dateInput.handleEvent(event, window);
            ...
            // Tab key to navigate between input fields
        ...
        // Update animations
        for (int i = 0; i < portPulse.size(); i++) {
            if (portPulse[i] > 0) {
                portPulse[i] -= 0.05f;
        ...
        for (auto& ripple : waterRipples) {
            float scale = 1.0f + 0.1f * sin(time * 0.5f + ripple.getPosition().x * 0.01f);
            ripple.setScale(scale, scale);
            Color c = ripple.getFillColor();
            c.a = 20 + 10 * sin(time * 0.3f + ripple.getPosition().y * 0.01f);
            ripple.setFillColor(c);
        }
        ...
        // Draw searched route path with animation
        if (showSearchResult && currentPath.getSize() > 0) {
            float animationOffset = fmod(time * 2.0f, 1.0f);
            ...
            for (int i = 0; i < currentPath.getSize() - 1; i++) {
                ...
                float animPos = animationOffset;
                if (i > 0) animPos = fmod(time * 2.0f + i * 0.3f, 1.0f);
                
                Vector2f dotPos = start + (end - start) * animPos;
        ...
        // Draw routes from selected port (existing code remains the same)
        ...
        // Draw ports (existing code remains the same)
        for (int i = 0; i < numLocations; i++) {
        ...
        // Draw port labels (existing code remains the same)
        ...
        // Draw beautiful main search button with animations
        ...
        if (showInputWindow) {
            ...
            // Draw input window with subtle animation
            float windowPulse = 1.0f + 0.05f * sin(time * 4.0f);
            inputWindow.setScale(windowPulse, windowPulse);
            inputWindow.setOrigin(250, 200); // Center the scaling
            window.draw(inputWindow);
            inputWindow.setScale(1.0f, 1.0f); // Reset scale
            inputWindow.setOrigin(0, 0);
            ...
            // Draw popup buttons with enhanced hover effects
            ...
            // Draw search result in popup if any
            if (showSearchResult) {
                Text resultText;
        ...
        // Draw search result on main screen
        if (showSearchResult && !showInputWindow) {
            ...
            // Draw path details if route found
        ...
        // Draw hover info box (existing code remains the same)
